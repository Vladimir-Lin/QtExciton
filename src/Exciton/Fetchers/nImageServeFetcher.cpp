...
#define STARTPS "<img class='centred' src='"
#define ENDPS   "'"

N::ImageServeFetcher:: ImageServeFetcher ( Plan * p )
                     : TwoStepsFetcher   (        p )
{
  if (NotNull(plan))                        {
...
N::ImageServeFetcher::~ImageServeFetcher (void)
{
}

bool N::ImageServeFetcher::VerifyUrl(QUrl & url)
{
  QString imgserve   = "http://imgserve.net/img-"                       ;
  QString damimage   = "http://damimage.com/img-"                       ;
  QString imagefolks = "http://www.imagefolks.com/img-"                 ;
  QString urlstring = url.toString()                                    ;
  bool    correct   = false                                             ;
  if ( ( ! correct ) && urlstring.contains(imgserve  ) ) correct = true ;
  if ( ( ! correct ) && urlstring.contains(damimage  ) ) correct = true ;
  if ( ( ! correct ) && urlstring.contains(imagefolks) ) correct = true ;
  return correct                                                        ;
}

bool N::ImageServeFetcher::Obtain (
       SqlConnection & SC         ,
       QUrl            url        ,
       SUID            puid       ,
       SUID          & ruid       ,
       ...
       int             mtimeout  )
{
  QString imgserve  = "http://imgserve.net/img-"                                ;
  QString damimage  = "http://damimage.com/img-"                                ;
  QString nexturl   = "<img class='centred' src='"                              ;
  QString urlstring = url.toString()                                            ;
  bool    correct   = false                                                     ;
  if ( ( ! correct ) && urlstring.contains(imgserve) ) correct = true           ;
  if ( ( ! correct ) && urlstring.contains(damimage) ) correct = true           ;
